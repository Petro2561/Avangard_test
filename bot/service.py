...
async def get_coin_by_name(coin_name: str, user_id: int, session: AsyncSession = None):
    async for session_inner in get_async_session():
        session = session if session else session_inner
